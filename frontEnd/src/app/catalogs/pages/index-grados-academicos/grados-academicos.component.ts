...
export class GradoAcademicoComponent {
    ...
    ngAfterViewInit(): void {
        this.route.queryParamMap.subscribe((params) => {
            ...
            if (typeMessage === '1') {
                this.snackbar.show('Grado académico creado correctamente', 3000);
            } else if (typeMessage === '2') {
                this.snackbar.show('Grado académico editado correctamente', 3000);
            } else if (typeMessage === '3') {
                this.snackbar.show('Grado académico eliminado correctamente', 3000);
            }
    ...
    eliminarGradoAcademico(confirmed: boolean): void {
        this.showModal = false;
        if (confirmed) {
            this.cargando = true;

            this.collectionsService.deleteGradoAcademico(this.selectedIdGrado).subscribe({
                next: (response) => {
                    this.cargando = false;
                    ...
                    if (response.status === 200) {
                        ...
                        console.error('Error al eliminar el grado académico:', response);
                        this.snackbar.show('Error al eliminar el grado académico', 3000);
                    }
                },
                error: (error) => {
    ...
    getTotalItems() {
        this.totalItems = this.gradosAcademicos.length > 1 ? this.gradosAcademicos.length : 0;
    }

    actualizarTabla() {
        const start = (this.currentPage - 1) * this.limitPerPage;
        const end = start + this.limitPerPage;
        const paginados = this.gradosAcademicos.slice(start, end);

        this.tableData = [...this.headers, ...paginados];
    }

    onDeleteRow(id: string) {
        this.selectedIdGrado = id;
        this.confirmUpdate();
    }
